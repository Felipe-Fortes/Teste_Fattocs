## Funcionalidades

- **Lista de Tarefas:** listagem ordenada pelo campo “Ordem de apresentação”; tarefas com custo ≥ R$ 1.000,00 em destaque (fundo amarelo); rodapé com somatório dos custos.
- **Incluir:** nova tarefa (nome, custo, data-limite obrigatórios); identificador e ordem gerados automaticamente; não permite nome duplicado.
- **Editar:** em popup; alteração de nome, custo e data-limite; não permite nome já existente.
- **Excluir:** com confirmação (Sim/Não).
- **Reordenar:** botões “Subir” e “Descer” em cada linha para alterar a ordem de apresentação.

Datas e números seguem o padrão brasileiro (DD/MM/AAAA e vírgula como separador decimal).
